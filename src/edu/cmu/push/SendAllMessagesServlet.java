...
public class SendAllMessagesServlet extends BaseServlet {
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp)
    ...
    if (devices.isEmpty()) {
      ...
      Queue queue = QueueFactory.getDefaultQueue();//QueueFactory.getQueue("gcm");
      ...
      if (devices.size() == 1) {
        ...
        queue.add(withUrl("/send").param(
            SendMessageServlet.PARAMETER_DEVICE, device).param("message",message));
        ...
        for (String device : devices) {
          ...
          if (partialSize == Datastore.MULTICAST_SIZE || counter == total) {
            ...
            TaskOptions taskOptions = TaskOptions.Builder
                .withUrl("/send")
                .param(SendMessageServlet.PARAMETER_MULTICAST, multicastKey).param("message",message)
                .method(Method.POST);
