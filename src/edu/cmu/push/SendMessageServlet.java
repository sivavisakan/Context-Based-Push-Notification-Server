...
public class SendMessageServlet extends BaseServlet {
  ...
  protected Sender newSender(ServletConfig config) {
    ...
    return new Sender("AIzaSyCB50hIjBNB_eP8oJet8lRNs7ZnPOM0Uyo");
  }
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp)
    ...
    String pushMessage = req.getParameter("message");
    if (regId != null) {
      sendSingleMessage(regId,pushMessage, resp);
      return;
    }
    String multicastKey = req.getParameter(PARAMETER_MULTICAST);
    if (multicastKey != null) {
      sendMulticastMessage(multicastKey,pushMessage, resp);
      return;
    }
  ...
  private void sendSingleMessage(String regId,String pushMessage, HttpServletResponse resp) {
    logger.info("Sending message to device " + regId);
    Message message = new Message.Builder().addData("msg", pushMessage).delayWhileIdle(false).build();
    Result result;
    try {
      result = sender.send(message, regId, 5);//sender.sendNoRetry(message, regId);
    } catch (IOException e) {
  ...
  private void sendMulticastMessage(String multicastKey,String pushMessage,
